In a multi-container pod, each container is expected to run a process that stays alive as long as the PODs lifecycle.
For example in the multi-container pod that has a web application and logging agent,
...
For example a process that pulls a code or binary from a repository
...
Thats where initContainers comes in.
